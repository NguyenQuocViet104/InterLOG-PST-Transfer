...
Phiên bản dashboard `0.3.2` dành cho IT quản lý toàn bộ vòng đời một yêu cầu backup PST: tiếp nhận yêu cầu, chọn đúng phạm vi, lên lịch export trên VM, xác nhận PST, theo dõi chuyển file và lưu lịch sử trong SQLite.
...
Nếu chưa nối receipt tự động, dashboard dùng quy trình xác nhận thủ công trung thực: `PST_READY` chỉ có nghĩa PST đang ở VM; IT sang máy user chạy PST Transfer, xác nhận `file đã về máy user`, sau đó mở PST bằng Outlook và xác nhận kiểm tra xong mới chuyển `COMPLETE`.
